...
local_cache <- fs::path_wd("_cache", "crawler", "repo")
remote_cache <- "s3://tidylab/github.observatory/queries/repo"

s3 <- S3::S3$new()
ecos <- Ecosystem$new()
repo_db <- RepoQueryDB$new(path = local_cache)
repo_queue <- collections::priority_queue(items = "dplyr")
...
# Upload Cache ------------------------------------------------------------
s3$dir_copy(local_cache, remote_cache, overwrite = TRUE)
