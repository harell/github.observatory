...
for(package in packages){ try(pb$tick(1), silent = TRUE); tryCatch({
    ...
    repo_overview <- observatory$query$packaobservatory$overview(owner, repo)
    repo_archive$save(repo_overview, tags = c("entity:repo", "type:overview", paste0("id:", repo_overview$id)))

    repo_contributors <- observatory$query$packaobservatory$contributors(owner, repo) |> purrr::map(~purrr::keep(.x, names(.x) %in% c("login", "id")))
    ...
    if(repo_overview$stargazers_count > 0) (
        observatory$query$packaobservatory$stargazers(owner, repo)
    ...
    if(repo_overview$subscribers_count > 0) (
        observatory$query$packaobservatory$watchers(owner, repo)
        |> purrr::map(~purrr::keep(.x, names(.x) %in% c("login", "id")))
